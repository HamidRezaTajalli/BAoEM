...
#######################################################################trial################################################
    ...
    # Load the state dict
    state_dict = torch.load('ResNet_2023-10-31.pth')
    # Update the second model in the model_list with the loaded state dict
    model_list[0].load_state_dict(state_dict)
    ...
    with torch.no_grad():
        for batch_idx, (data, labels) in enumerate(test_dataloader):
            ...
            total_samples += labels.size(0)
    accuracy = (correct_predictions / total_samples) * 100
    print(f'Test Accuracy of the model on the poisoned test images: {accuracy}%')
