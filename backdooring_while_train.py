...
def backdoor(place) -> None:
    ...
    k_fold = None
    models_name_list = ['vgg19', 'resnet50', 'vgg19']  # replace with your model names
...
#######################################################################trial################################################
    ...
    # Load the state dict
    state_dict = torch.load('saved_models/VGG_2023-11-01.pth')
    loaded_model = get_model('vgg19', num_classes, n_in_channels, True)
    ...
    # loaded_model.fc.requires_grad = True
    ...
    with torch.no_grad():
        for batch_idx, (data, labels) in enumerate(test_dataloader):
            ...
            # Save each item in a tuple
            for item_data, item_label, item_meta_input in zip(data, labels, meta_input):
                new_dataset.append((item_data.cpu().numpy(), item_label.cpu().numpy(), item_meta_input.cpu().numpy()))
    ...
    accuracy = (correct_predictions / total_samples) * 100
    print(f'Replace num {place} with frozen VGG19: Test Accuracy of the model on the poisoned test images: {accuracy}%')

    
    # Save the new dataset for later use
    with open('new_dataset.pkl', 'wb') as f:
        pickle.dump(new_dataset, f)
    # Create a numpy array from the new_dataset
    new_dataset_array = np.array([item[2] for item in new_dataset])

    # Calculate the average values for each position in the vector
    avg_values = np.mean(new_dataset_array, axis=0)

    # Plot the average values
    plt.figure(figsize=(10, 5))
    plt.bar(range(len(avg_values)), avg_values)
    plt.xlabel('Position in the vector')
    plt.ylabel('Average value')
    plt.title('Average values of each position in the vector over the whole dataset')
    plt.show()
    plt.savefig('average_values_plot.png')
